# PA-TEN-005-API-01-Suspend moto tenant

| Hạng mục | Nội dung |
| --- | --- |
| API Name | Suspend moto tenant |
| Description | Tạm ngưng tenant MOTO. |
| Endpoint | `/api/v1/admin/moto-tenants/{id}/suspend` |
| Menu | Tenant Management |
| Method | PATCH |
| Related Tables | `platform.tenant_registry` |
| Screen list | 派遣元テナント停止 (PA-TEN-005) — Suspend MOTO Tenant |
...
| Tên tài liệu | API Detail Design - Suspend moto tenant |
...
| --- | --- | --- | --- |
| v1.0 | 2026/06/17 | Thuận | Khởi tạo tài liệu đặc tả chi tiết API Suspend MOTO Tenant dựa trên thiết kế màn hình, cấu trúc DB thực tế và API Restore tương ứng |
...
| Hạng mục | Nội dung |
| --- | --- |
| API ID | PA-TEN-005-API-01 |
| Tên API | Suspend moto tenant |
| Business Flow liên quan | BF-003 Tenant Management Flow |
| Screen ID liên quan | PA-TEN-005 / PA-TEN-001 |
| Chức năng liên quan | Tenant Management (Suspend MOTO Tenant) |
| Mục đích API | Tạm ngưng hoạt động của một MOTO Tenant (từ Hoạt động sang Tạm ngưng). |
...
API này dùng để tạm ngưng hoạt động của một MOTO Tenant (派遣元テナント), ngăn người dùng của Tenant đó đăng nhập và sử dụng hệ thống cho đến khi được khôi phục.

### Ngữ cảnh nghiệp vụ

API được gọi khi Platform SaaS Admin nhấn nút "停止" (Tạm ngưng) trên màn hình danh sách MOTO Tenant (PA-TEN-001) hoặc từ màn hình tạm ngưng Tenant MOTO (PA-TEN-005).

| Hạng mục | Nội dung |
| --- | --- |
| Hành động kích hoạt | User nhấn nút Suspend Tenant trên giao diện màn hình PA-TEN-001 hoặc PA-TEN-005, xác nhận qua dialog `CMS-VAL-85` |
...
| --- | --- | --- | --- | --- |
| id | string | Có | ID của tenant cần tạm ngưng (khóa chính `tenant_id` - ULID) | 01H2PJX7G3P681729X4R09Q871 |
...
```json
{
  "error": {
    ...
    "message": "The tenant is already in suspended status."
  }
}
```
...
| --- | --- | --- | --- | --- |
| 1 | id | Bắt buộc, phải đúng định dạng ULID (26 ký tự chữ và số) | CMS-VAL-23 | idを入力してください。 |
...
> **Ghi chú:** Theo ERD thực tế của dự án, dữ liệu tenant được quản lý tại bảng `platform.tenant_registry` (schema `platform`).
...
| Wireframe | PA-TEN-005 Suspend MOTO Tenant |
| Screen Detail Design | PA-TEN-005 |
| API liên quan | PA-TEN-001-API-01 Moto tenant list, PA-TEN-006-API-01 Restore moto tenant |
