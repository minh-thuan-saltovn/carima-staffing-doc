...
| Field | Type | Mô tả |
| --- | --- | --- |
| message | string | Thông báo thực hiện 停止 thành công |
...
| BR-003 | Authorization check | Chỉ người dùng thuộc nhóm Platform SaaS Admin có quyền `platform.tenant.suspend_saki_tenant.cancel` mới được phép gọi API này. |
...
4. Kiểm tra quyền platform.tenant.suspend_saki_tenant.cancel
...
3. Middleware kiểm tra quyền truy cập (Authorization): Platform SaaS Admin có quyền "platform.tenant.suspend_saki_tenant.cancel".
...
| No. | Hạng mục | Mô tả |
| --- | --- | --- |
| 1 | Authentication & Authorization | Bắt buộc kiểm tra token hợp lệ và phân quyền Platform SaaS Admin (`platform.tenant.suspend_saki_tenant.cancel`). |
