# PA-TEN-003-API-01-Create moto tenant

| Hạng mục | Nội dung |
| --- | --- |
| API Name | Create moto tenant |
| Description | Tạo mới tenant MOTO. |
| Endpoint | `/api/v1/admin/moto-tenants` |
| Menu | Tenant Management |
| Method | POST |
| Related Tables | `platform.tenant_registry`, `tenant_db.mst_moto_company`, `tenant_db.mst_moto_user` |
| Screen list | 派遣元テナント作成 (PA-TEN-003) — Create MOTO Tenant |
...
| Tên tài liệu | API Detail Design - Create moto tenant |
...
| --- | --- | --- | --- |
| v1.0 | 2026/06/17 | Thuận | Khởi tạo tài liệu đặc tả chi tiết API Create MOTO Tenant theo template, sơ đồ màn hình và cấu trúc DB thực tế |
...
| Hạng mục | Nội dung |
| --- | --- |
| API ID | PA-TEN-003-API-01 |
| Tên API | Create moto tenant |
| Business Flow liên quan | BF-003 Tenant Management Flow |
| Screen ID liên quan | PA-TEN-003 |
| Chức năng liên quan | Tenant Management (Create MOTO Tenant) |
| Mục đích API | Đăng ký một MOTO Tenant (doanh nghiệp cung ứng nhân lực) mới vào hệ thống, tự động cấp phát schema/database PostgreSQL riêng và khởi tạo tài khoản Admin cho Tenant đó. |
...
API này được sử dụng bởi quản trị viên hệ thống (Platform SaaS Admin) để đăng ký một Tenant MOTO mới. Khi đăng ký thành công, hệ thống sẽ tự động khởi tạo cơ sở dữ liệu riêng (PostgreSQL schema riêng), phân quyền người dùng quản trị ban đầu, và gửi email chứa liên kết thiết lập mật khẩu đến địa chỉ email của quản trị viên tenant.

### Ngữ cảnh nghiệp vụ

Quản trị viên điền đầy đủ thông tin vào form của màn hình "Tạo Tenant MOTO mới" (PA-TEN-003), sau đó nhấn nút "MOTOテナント発行" (Phát hành) để kích hoạt quá trình.

| Hạng mục | Nội dung |
| --- | --- |
| Hành động kích hoạt | User nhấn nút Phát hành trên giao diện màn hình PA-TEN-003, xác nhận qua dialog `CMS-VAL-85` |
...
```json
{
  "data": {
    ...
    "created_at": "2026-06-17T14:33:00+09:00"
  }
}
```
...
| - | platform.tenant_registry | tenant_type | `'moto'` |
...
| - | platform.tenant_registry | schema_name | System Generated: `tenant_moto_{tenant_code_lowercase}` |
| - | platform.tenant_registry | db_user | System Generated: `dbuser_moto_{tenant_code_lowercase}` |
...
**Bảng thông tin doanh nghiệp trong schema riêng của Tenant: `tenant_db.mst_moto_company`**

| Request Field | Table | Column | Logic mapping / Giá trị |
| --- | --- | --- | --- |
| - | tenant_db.mst_moto_company | company_id | Tự phát sinh theo format: `MOTO-{4 chữ số tự tăng}` |
| official_name_ja | tenant_db.mst_moto_company | official_name_ja | `official_name_ja` |
| - | tenant_db.mst_moto_company | official_name_kana | `official_name_ja` chuyển sang Hiragana/Katakana nếu cần (hoặc mặc định rỗng/lấy từ text) |
| display_name_ja | tenant_db.mst_moto_company | display_name_ja | `display_name_ja` |
| postal_code | tenant_db.mst_moto_company | postal_code | `postal_code` |
| address_ja | tenant_db.mst_moto_company | address_ja | `address_ja` |
| - | tenant_db.mst_moto_company | official_name_en | Chuyển đổi không dấu hoặc rỗng |
| - | tenant_db.mst_moto_company | display_name_en | Chuyển đổi không dấu hoặc rỗng |
| - | tenant_db.mst_moto_company | created_by | ID của Platform Admin thực hiện |
| - | tenant_db.mst_moto_company | updated_by | ID của Platform Admin thực hiện |

**Bảng tài khoản quản trị ban đầu của Tenant: `tenant_db.mst_moto_user`**

| Request Field | Table | Column | Logic mapping / Giá trị |
| --- | --- | --- | --- |
| - | tenant_db.mst_moto_user | user_id | Tự phát sinh mã người dùng (ULID hoặc VARCHAR định danh) |
| - | tenant_db.mst_moto_user | company_id | Liên kết khóa ngoại `company_id` vừa tạo phía trên |
| admin_user.last_name_ja | tenant_db.mst_moto_user | last_name_ja | `admin_user.last_name_ja` |
| admin_user.first_name_ja | tenant_db.mst_moto_user | first_name_ja | `admin_user.first_name_ja` |
| - | tenant_db.mst_moto_user | last_name_kana | Mặc định rỗng hoặc convert |
| - | tenant_db.mst_moto_user | first_name_kana | Mặc định rỗng hoặc convert |
| - | tenant_db.mst_moto_user | last_name_en | Mặc định rỗng hoặc convert |
| - | tenant_db.mst_moto_user | first_name_en | Mặc định rỗng hoặc convert |
| admin_user.tel | tenant_db.mst_moto_user | tel | `admin_user.tel` |
| admin_user.email | tenant_db.mst_moto_user | email | `admin_user.email` |
| - | tenant_db.mst_moto_user | status | `1` (Hoạt động) |
| - | tenant_db.mst_moto_user | reference_scope | `5` (Toàn quyền hiển thị) |
| - | tenant_db.mst_moto_user | execution_role | `1` (Vai trò Admin cao nhất) |
| - | tenant_db.mst_moto_user | created_by | ID của Platform Admin thực hiện |
| - | tenant_db.mst_moto_user | updated_by | ID của Platform Admin thực hiện |
...
| 7 | INSERT bản ghi thông tin công ty vào `mst_moto_company` của schema mới | Trong transaction |
| 8 | INSERT bản ghi tài khoản admin ban đầu vào `mst_moto_user` của schema mới | Trong transaction |
...
| --- | --- | --- | --- |
| (Không tồn tại) | POST /admin/moto-tenants | 1 (Hoạt động) | Đăng ký thành công |
...
5. Controller gọi Service thực hiện nghiệp vụ đăng ký trong Database Transaction:
   ...
   - Thực thi tạo schema PostgreSQL mới (`tenant_moto_{code}`) và tạo user DB riêng.
   - Chạy lệnh migration hệ thống tự động để tạo các bảng nghiệp vụ trong schema mới.
   - INSERT dữ liệu doanh nghiệp mới vào bảng `mst_moto_company` thuộc schema vừa tạo.
   - INSERT dữ liệu tài khoản quản trị viên ban đầu vào bảng `mst_moto_user` thuộc schema mới.
...
| Đối tượng nhận | Email quản trị viên của Tenant MOTO mới (`admin_user.email`) |
| Kênh gửi | Email tự động |
| Template ID | platform.email.moto_admin_activation |
...
| Hạng mục | Target |
| --- | --- |
| Thời gian response kỳ vọng | Trong vòng 2.0 giây (do phải chạymigrations động khi tạo schema) |
...
| Wireframe | PA-TEN-003 Create MOTO Tenant |
| Screen Detail Design | PA-TEN-003 |
