# PA-TEN-012-API-01-Restore saki tenant

| Hạng mục | Nội dung |
| --- | --- |
| API Name | Restore saki tenant |
| Description | Khôi phục tenant SAKI. |
| Endpoint | `/api/v1/admin/saki-tenants/{id}/restore` |
| Menu | Tenant Management |
| Method | PATCH |
| Related Tables | `platform.tenant_registry` |
| Screen list | 派遣先テナント復旧 (PA-TEN-012) — Restore SAKI Tenant |
...
| Tên tài liệu | API Detail Design - Restore saki tenant |
...
| --- | --- | --- | --- |
| v1.0 | 2026/06/17 | Thuận | Khởi tạo tài liệu đặc tả chi tiết API Restore saki tenant theo template và cấu trúc DB thực tế |
...
| Hạng mục | Nội dung |
| --- | --- |
| API ID | PA-TEN-012-API-01 |
| Tên API | Restore saki tenant |
| Business Flow liên quan | BF-003 Tenant Management Flow |
| Screen ID liên quan | PA-TEN-012 / PA-TEN-007 |
| Chức năng liên quan | Tenant Management (Restore SAKI Tenant) |
| Mục đích API | Khôi phục trạng thái hoạt động của một SAKI Tenant (từ Tạm ngưng sang Hoạt động). |
...
API này dùng để khôi phục trạng thái hoạt động cho một SAKI Tenant (派遣先テナント) đã bị tạm ngưng (suspend) trước đó, cho phép người dùng của Tenant đó đăng nhập lại và sử dụng bình thường.

### Ngữ cảnh nghiệp vụ

API được gọi khi Platform SaaS Admin nhấn nút "復旧" (Khôi phục) trên màn hình danh sách SAKI Tenant (PA-TEN-007) hoặc từ màn hình khôi phục Tenant SAKI (PA-TEN-012).

| Hạng mục | Nội dung |
| --- | --- |
| Hành động kích hoạt | User nhấn nút Restore Tenant trên giao diện màn hình PA-TEN-007 hoặc PA-TEN-012, xác nhận qua dialog `CMS-VAL-85` |
...
| --- | --- | --- | --- | --- |
| id | string | Có | ID của tenant cần khôi phục (khóa chính `tenant_id` - ULID) | 01H2PJX7G3P681729X4R09Q872 |
...
| --- | --- | --- | --- |
| platform.tenant_registry | status | `1` | Chuyển trạng thái sang Hoạt động (有効) |
...
5. Controller gọi Service thực hiện nghiệp vụ khôi phục trong một Database Transaction:
   ...
   - Thực hiện cập nhật bản ghi: `status = 1`, `updated_at = now()`.
...
| Wireframe | PA-TEN-012 Restore SAKI Tenant |
| Screen Detail Design | PA-TEN-012 |
| API liên quan | PA-TEN-007-API-01 Saki tenant list, PA-TEN-011-API-01 Suspend saki tenant |
