# PA-TEN-001-API-01-Moto tenant list
...
| Tên tài liệu | API Detail Design - Moto tenant list |
...
| --- | --- | --- | --- |
| v1.0 | 2026/06/16 | Thuận | Khởi tạo tài liệu đặc tả chi tiết API Moto tenant list theo template chuẩn của dự án |
| v1.1 | 2026/06/17 | Thuận | Cập nhật cấu trúc DB thực tế (bảng tenant_registry, t_tenant_usage_metric) và trường response khớp OpenAPI schemas |
...
| Hạng mục | Nội dung |
| --- | --- |
| API ID | PA-TEN-001-API-01 |
| Tên API | Moto tenant list |
| Business Flow liên quan | BF-003 Tenant Management Flow |
| Screen ID liên quan | PA-TEN-001 |
| Chức năng liên quan | Tenant Management (MOTO Tenant List) |
| Mục đích API | Lấy danh sách tenant MOTO, hỗ trợ tìm kiếm, lọc và phân trang. |
...
API này dùng để lấy danh sách tất cả các MOTO Tenant (派遣元テナント) trong hệ thống phục vụ cho việc hiển thị, tìm kiếm, lọc và phân trang tại màn hình quản lý Tenant của Platform SaaS Admin.

### Ngữ cảnh nghiệp vụ

API được gọi khi Platform SaaS Admin truy cập vào màn hình quản lý Tenant MOTO hoặc thực hiện thao tác lọc/tìm kiếm/chuyển trang trên màn hình này.
...
| Trước khi gọi API | Đã đăng nhập vào hệ thống Platform Manager và có quyền `platform.tenant.moto_tenant_list.view` |
| Sau khi gọi API | Hiển thị danh sách tenant MOTO lên bảng dữ liệu cùng thông tin phân trang |
...
| tenant_code | string | Không | null | Lọc theo mã Tenant (tìm kiếm tương đối) | MOTO-0001 |
...
| Field | Type | Mô tả |
| --- | --- | --- |
| data[] | array | Danh sách MOTO Tenants |
...
| data[].company_name | string | Tên doanh nghiệp phái cử MOTO |
...
| data[].active_staff_count | int | Số lượng nhân viên đang hoạt động |
| data[].active_contract_count | int | Số lượng hợp đồng phái cử đang có hiệu lực |
| data[].billing_status | tinyint | Trạng thái thanh toán (0: 未払い - Chưa thanh toán, 1: 支払い済 - Đã thanh toán) |
...
| No. | Rule | Mô tả |
| --- | --- | --- |
| BR-001 | Tenant type restriction | API này chỉ được phép trả về các tenant có loại `tenant_type = 'moto'` (MOTO Tenant). |
| BR-002 | Role authorization | Chỉ người dùng thuộc nhóm quản trị Platform (SaaS Admin / SaaS Staff) có quyền `platform.tenant.moto_tenant_list.view` mới được phép thực hiện gọi API này. |
...
| BR-005 | Dynamically calculated counts | `active_staff_count`, `active_contract_count` và `billing_status` được lấy từ bảng cache thống kê hoạt động `platform.t_tenant_usage_metric` (hoặc tính toán join tương ứng) để tránh truy vấn trực tiếp vào database riêng của từng tenant, nhằm đảm bảo hiệu năng. |
...
3. Kiểm tra xem user có thuộc hệ thống quản trị Platform Manager và có permission "platform.tenant.moto_tenant_list.view" hay không.
...
| platform.t_tenant_usage_metric | metric_value | data.items[].active_staff_count | Số lượng staff đang hoạt động. Lọc theo `target_year_month = :current_ym` và `metric_type = 'active_staff_count'` |
...
```
1. Client gửi request: GET /api/v1/admin/moto-tenants kèm query parameters.
2. Middleware thực hiện kiểm tra Authentication và xác định JWT Token hợp lệ.
3. Middleware thực hiện kiểm tra Authorization (quyền "platform.tenant.moto_tenant_list.view").
...
5. Controller gọi Service để lấy dữ liệu:
   - Xây dựng Query Builder truy vấn bảng `platform.tenant_registry` với điều kiện `tenant_type = 'moto'`.
...
| No. | Hạng mục | Mô tả |
| --- | --- | --- |
| 1 | Authentication & Authorization | Bắt buộc kiểm tra token hợp lệ và phân quyền Platform SaaS Admin/Staff (`platform.tenant.moto_tenant_list.view`). |
...
| API liên quan | PA-TEN-001-API-02 Export moto tenant list |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Xác nhận cấu trúc bảng `tenant_usage_metric` và tần suất cập nhật dữ liệu thống kê staff/contract. | Medium | Platform Lead | TBD | Open |
