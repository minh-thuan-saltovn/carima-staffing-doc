...
| Tên tài liệu | API Detail Design - Unlock Platform User |
...
| Hạng mục | Nội dung |
| --- | --- |
| API ID | PA-USER-005-API-03 |
| Tên API | Unlock Platform User |
...
| Mục đích API | Thực hiện thao tác mở khóa tài khoản quản trị viên Platform |
| Loại API | Frontend API |
| Method | POST |
| Endpoint | /api/v1/admin/users/{id}/unlock |
| Authentication | Có |
| Authorization | Có (platform.user.account_lock_unlock.unlock) |
...
API này dùng để mở khóa tài khoản của một quản trị viên Platform cụ thể, đặt lại cờ `account_locked_flg` = 0, xóa số lần đăng nhập sai `login_fail_count` = 0, ghi log hoạt động lý lý do mở khóa của Admin, và gửi thông báo email cho người dùng mục tiêu.

### Ngữ cảnh nghiệp vụ

| Hạng mục | Nội dung |
| --- | --- |
| Hành động kích hoạt | Admin chọn lý do và bấm nút "Mở khóa tài khoản" |
| Actor | Platform SaaS Admin |
| Trước khi gọi API | Client hiển thị modal và điền lý do mở khóa |
| Sau khi gọi API | Tài khoản được mở khóa, log lý do được ghi nhận, gửi email thông báo |
| Thay đổi trạng thái liên quan | Cập nhật account_locked_flg = 0, login_fail_count = 0 |
...
| --- | --- | --- | --- | --- |
| id | number | Có | ID của người dùng cần mở khóa tài khoản | 1001 |
...
```json
{
  "unlock_reason": "Mở khóa theo yêu cầu của phòng nhân sự",
...
| --- | --- | --- | --- | --- | --- | --- |
| unlock_reason | - | string | Có | 1000 | Không được rỗng | Lý do thực hiện mở khóa tài khoản |
...
| --- | --- | --- |
| 200 OK | - | Mở khóa tài khoản thành công |
| 400 Bad Request | VAL001 | Tham số đầu vào không đúng định dạng hoặc thiếu lý do mở khóa |
| 401 Unauthorized | AUTH001 | Token không hợp lệ hoặc đã hết hạn |
| 403 Forbidden | AUTH002 | Không có quyền thực hiện hành động mở khóa tài khoản |
| 404 Not Found | USR001 | Không tìm thấy người dùng cần mở khóa |
...
| --- | --- | --- | --- | --- |
| 1 | unlock_reason | required | - | unlock_reason_required |
| 2 | unlock_reason | max | 1000 | unlock_reason_too_long |

# 8. Business Rules

| No. | Rule | Mô tả |
| --- | --- | --- |
| BR-001 | Unlock status update | Set cờ `account_locked_flg` = 0, `login_fail_count` = 0, và xóa thông tin `locked_at`, `locked_until` trong bảng `mst_platform_user`. |
| BR-002 | Audit Logging | Bắt buộc ghi nhận log lý do mở khóa (`unlock_reason`) cùng ID của Admin thao tác vào bảng log vận hành hệ thống. |
| BR-003 | Email sending | Nếu `send_notification_email` = true, hệ thống tự động gửi email thông báo tài khoản đã được mở khóa cho user mục tiêu. |
| BR-004 | Admin check | Quản trị viên Platform có quyền mở khóa cho các tài khoản quản trị viên khác. |
...
2. Kiểm tra quyền platform.user.account_lock_unlock.unlock liên kết với Role.
...
```
1. Frontend gửi yêu cầu mở khóa (POST /api/v1/admin/users/{id}/unlock).
2. Backend xác thực JWT token và kiểm tra quyền.
3. Backend validate thông tin unlock_reason.
4. Backend kiểm tra sự tồn tại của user_id trong mst_platform_user.
5. Backend khởi tạo Transaction trên central_db.
6. Backend cập nhật account_locked_flg = 0, login_fail_count = 0, locked_at = null, locked_until = null trong mst_platform_user.
7. Backend chèn log lý do unlock_reason vào bảng log hoạt động.
8. Backend commit Transaction.
9. Backend gửi mail thông báo cho user (nếu send_notification_email = true).
10. Backend trả Response thành công 200 OK.
```
