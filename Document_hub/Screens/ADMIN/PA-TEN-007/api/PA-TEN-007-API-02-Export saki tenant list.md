# PA-TEN-007-API-02-Export saki tenant list

| Hạng mục | Nội dung |
| --- | --- |
| API Name | Export saki tenant list |
| Description | Xuất danh sách tenant SAKI ra file. |
| Endpoint | `/api/v1/admin/saki-tenants/export` |
...
| Tên tài liệu | API Detail Design - Export saki tenant list |
...
| --- | --- | --- | --- |
| v1.0 | 2026/06/17 | Thuận | Khởi tạo tài liệu đặc tả chi tiết API Export saki tenant list theo template và cấu trúc DB thực tế |
...
| Hạng mục | Nội dung |
| --- | --- |
| API ID | PA-TEN-007-API-02 |
| Tên API | Export saki tenant list |
| Business Flow liên quan | BF-003 Tenant Management Flow |
| Screen ID liên quan | PA-TEN-007 |
| Chức năng liên quan | Tenant Management (SAKI Tenant List - Export) |
| Mục đích API | Xuất danh sách tenant SAKI ra file CSV. |
...
API này dùng để xuất danh sách tất cả hoặc danh sách đã lọc các SAKI Tenant (派遣先テナント) ra tệp tin định dạng CSV phục vụ cho việc lưu trữ, báo cáo của Platform SaaS Admin.

### Ngữ cảnh nghiệp vụ

API được gọi khi Platform SaaS Admin nhấn nút "Xuất file" (CSV) trên màn hình quản lý Tenant SAKI. Bộ lọc tìm kiếm hiện tại trên màn hình sẽ được gửi kèm để xuất đúng dữ liệu đang được lọc.

| Hạng mục | Nội dung |
| --- | --- |
| Hành động kích hoạt | User nhấn nút Export CSV trên giao diện màn hình PA-TEN-007 |
| Actor | Platform SaaS Admin / Platform SaaS Staff |
| Trước khi gọi API | Đã đăng nhập vào hệ thống Platform Manager và có quyền `platform.tenant.saki_tenant_list.view` / `tenant.export` |
| Sau khi gọi API | Tải tệp tin CSV chứa danh sách Tenant SAKI về máy của Client |
...
| Content-Disposition | attachment; filename="saki_tenants_export_YYYYMMDD.csv" | Tên tệp tin tải về mặc định |

### Response Body

```csv
"ID Tenant","Mã Tenant","Tên Doanh Nghiệp","Schema Name","DB User","Gói Dịch Vụ","Ngày Ký Hợp Đồng","Số MOTO Kết Nối","Số Hợp Đồng","Số Người Phê Duyệt","Trạng Thái Thanh Toán","Trạng Thái","Ngày Tạo"
"01H2PJX7G3P681729X4R09Q872","SAKI-0001","SAKI Partner Factory","tenant_saki_0001","db_saki_user_0001","ENTERPRISE","2026-04-01",5,12,8,"Đã thanh toán","Hoạt động","2026-04-01 00:00:00"
```
...
| 3 | Tên Doanh Nghiệp | `platform.tenant_registry.company_name` | Tên doanh nghiệp SAKI |
...
| 8 | Số MOTO Kết Nối | `platform.mst_moto_saki_relation.count` | Số lượng MOTO connect (Tính toán đếm) |
...
| 10 | Số Người Phê Duyệt | `platform.t_tenant_usage_metric.metric_value` | Số tài khoản người phê duyệt (Lọc theo `target_year_month = :current_ym` và `metric_type = 'approval_user_count'`) |
| 11 | Trạng Thái Thanh Toán | `platform.t_usage_billing.billing_status` | Trạng thái thanh toán phí dịch vụ (Lọc theo `target_year_month = :current_ym` và ánh xạ giá trị) |
| 12 | Trạng Thái | `platform.tenant_registry.status` | 0: Tạm ngưng, 1: Hoạt động |
| 13 | Ngày Tạo | `platform.tenant_registry.created_at` | Ngày tạo bản ghi hệ thống |
...
| No. | Rule | Mô tả |
| --- | --- | --- |
| BR-001 | Tenant type restriction | Chỉ xuất các tenant có `tenant_type = 'saki'` (SAKI Tenant). |
| BR-002 | Role authorization | Chỉ người dùng thuộc nhóm quản trị Platform (SaaS Admin / SaaS Staff) có quyền `platform.tenant.saki_tenant_list.view` và `tenant.export` mới được gọi API này. |
...
| platform.mst_moto_saki_relation | count | Số MOTO Kết Nối | Số lượng MOTO connect |
...
| platform.t_tenant_usage_metric | metric_value | Số Người Phê Duyệt | Số tài khoản phê duyệt SAKI. Lọc theo `target_year_month = :current_ym` và `metric_type = 'approval_user_count'` |
...
```
1. Client gửi request: GET /api/v1/admin/saki-tenants/export kèm query parameters bộ lọc.
2. Middleware thực hiện kiểm tra Authentication và xác định JWT Token hợp lệ.
3. Middleware kiểm tra quyền truy cập: Platform SaaS Admin/Staff có quyền "platform.tenant.saki_tenant_list.view" và "tenant.export".
...
5. Controller gọi Service để lấy dữ liệu xuất bản:
   - Truy vấn bảng `platform.tenant_registry` với điều kiện `tenant_type = 'saki'`.
   - Thực hiện LEFT JOIN với bảng `platform.mst_moto_saki_relation` để lấy `connected_moto_count` (thông qua đếm gom nhóm).
   - Thực hiện LEFT JOIN với bảng `platform.t_tenant_usage_metric` trên trường `tenant_id` và `target_year_month = :current_ym` để lấy `active_contract_count` (khi `metric_type = 'active_contract_count'`) và `approval_user_count` (khi `metric_type = 'approval_user_count'`) thông qua xoay dòng (pivot/conditional aggregation).
...
| Dữ liệu ghi log | Ghi nhận nhật ký xuất file danh sách Tenant SAKI vào hệ thống audit |
...
| No. | Hạng mục | Mô tả |
| --- | --- | --- |
| 1 | Authentication & Authorization | Bắt buộc kiểm tra token hợp lệ và phân quyền Platform SaaS Admin/Staff (`platform.tenant.saki_tenant_list.view` và `tenant.export`). |
