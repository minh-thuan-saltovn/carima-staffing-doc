...
メニュー: 契約管理
Screen ID: MO-CON-001
Screen (VI): Contract List
Giải thích tính năng: Danh sách contract
機能説明: 契約一覧を表示する
Thông tin hiển thị trên màn hình: Contract No, client, staff, contract period, status
画面表示情報: 契約番号、取引先、スタッフ、契約期間、ステータス
URL: /moto/contracts
Business Flow: Diagram 2 — 派遣照会〜契約成立フロー (https://www.notion.so/Diagram-2-364f02c407dd806a8429dd8ff501068b?pvs=21), 契約作成フロー (https://www.notion.so/362f02c407dd80f3a10cfe645cd590f4?pvs=21), 契約承認フロー (https://www.notion.so/362f02c407dd80d9a3fef628f1cf05f6?pvs=21)
システム: 派遣元ポータル
API List: MO-CON-001-API-01-Get Contract List (https://www.notion.so/MO-CON-001-API-01-Get-Contract-List-36bf02c407dd8039bbbcefab20bdc59c?pvs=21)
Status: In progress
Wireframe list: Untitled (https://www.notion.so/374f02c407dd80939a74efa9880c590a?pvs=21)
...
| --- | --- |
| Screen ID | MO-CON-001 |
| Tên màn hình | Danh sách Hợp đồng phái cử |
| Tên tiếng Nhật | 契約一覧 |
| Module | Contract Management |
| Chức năng | Tìm kiếm, xem danh sách, kết thúc hợp đồng, copy và xuất dữ liệu hợp đồng phái cử |
...
- Tìm kiếm hợp đồng phái cử dựa trên các tiêu chí lọc (Mã hợp đồng, công ty tiếp nhận SAKI, nhân viên Staff, người phụ trách Sales, trạng thái hợp đồng, trạng thái phê duyệt, trạng thái hóa đơn, trạng thái chấm công, thời hạn hợp đồng).
...
- Hiển thị danh sách hợp đồng phái cử theo bộ lọc mặc định (hoặc điều kiện tìm kiếm được chỉ định).
...
| --- | --- |
| MO-SAKI-002 | Client Detail |
| MO-CON-002 | 契約詳細・編集 (Contract Detail・Edit) - Quay lại |
| MO-CON-003 | 契約作成 (Create Contract) - Quay lại |
| MO-CON-004 | 派遣照会から契約作成 (Create Contract From Request) - Quay lại |
| MO-CON-024 | 未提出契約一覧 (Unsubmitted Contract List) - Quay lại |
...
| Tạo mới (Từ yêu cầu) | MO-CON-004 | 派遣照会から契約作成 (Create Contract From Request) |
| Xem chi tiết & Chỉnh sửa | MO-CON-002 | 契約詳細・編集 (Contract Detail・Edit) |
| Xem danh sách Nháp | MO-CON-005 | 作成中契約一覧 (Draft Contract List) |
| Upload hợp đồng | MO-CON-008 | 契約アップロード (Contract Upload) |
| Xem danh sách chưa gửi | MO-CON-024 | 未提出契約一覧 (Unsubmitted Contract List) |
...
- Gồm 5 tab nằm ngay trên khu vực bộ lọc:
    - **契約一覧 (Danh sách hợp đồng):** Hiển thị toàn bộ hợp đồng. **Mặc định active ở màn hình này (gạch chân đen, in đậm).**
    ...
    - **未請求 (Chưa xuất hóa đơn):** Chỉ hiển thị các hợp đồng chưa lập hóa đơn.
...
| 7 | Trạng thái hóa đơn (請求状態) | Dropdown | enum | No | Trạng thái xuất hóa đơn (未請求, 請求済み...). Mặc định: "すべての請求状態" |
...
| 14 | Xuất dữ liệu (エクスポート) | Button | - | - | Xuất danh sách hợp đồng hiện tại theo bộ lọc ra file CSV |
...
3. Gửi Request gọi API `GET /api/v1/moto/contracts/contract-list` lên Server.
...
2. Thiết lập lại trạng thái active mặc định cho thẻ thống kê nhanh "Tổng hợp đồng" và menu tab "契約一覧".
3. Gọi lại API tìm kiếm hợp đồng không kèm điều kiện để tải lại danh sách hợp đồng mặc định (trang 1).
...
```
GET /api/v1/moto/contracts/contract-list
```
...
{
  "status": "success",
  "message": "get_contract_list_success",
  "data": {
    "items": [
      {
        "id": 82,
        ...
        "end_date": "2027/04/30",
        "billing_unit_price": 640000,
        "closing_status": "closed",
        "unapproved_attendance_count": 0,
        "unbilled_status": "billed",
        "extension_allowed": true,
        "sales_pic": "佐藤 花子",
        "updated_at": "2026-05-30 14:23:00"
      }
    ],
    "total": 156,
    "current_page": 1,
