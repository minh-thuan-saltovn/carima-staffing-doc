...
| Screen ID liên quan | SA-ATT-002 |
| Chức năng liên quan | Đăng xuất khỏi SAKI Portal |
...
API này dùng để đăng xuất người dùng SAKI Portal khỏi hệ thống.
...
| Actor | SAKI User |
| Trước khi gọi API | User đang đăng nhập SAKI Portal |
...
| BR-006 | User Existence Check | User của token phải tồn tại trong tenant_db.mst_saki_user |
...
| Role | Cho phép logout SAKI Portal | Ghi chú |
| --- | --- | --- |
| SAKI Admin | Có | Nếu token thuộc Tenant hiện tại |
| SAKI User / Staff | Có | Nếu token thuộc Tenant hiện tại |
| MOTO Admin | Không | Không thuộc SAKI Portal |
| MOTO User | Không | Không thuộc SAKI Portal |
...
| 7 | Kiểm tra user thuộc SAKI Portal |
| 8 | Cho phép logout |
...
| tenant_db | TBD: audit_log | target_table | mst_saki_user | Bảng target |
...
| tenant_db | mst_saki_user | idx_mst_saki_user_user_id | user_id | Tìm user |
...
10. Backend tìm user trong tenant_db.mst_saki_user
...
| Wireframe | SA-ATT-002 |
| Screen Detail Design | SA-ATT-002 |
| API Login | SA-AUTH-001-API-01-Login |
| ERD | Carima Data Dictionary |
| Data Dictionary | Carima-Staffing Data Dictionary / ERD (mst_tenant, mst_saki_user) |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Chưa xác nhận bảng Access Token chính thức trong Data Dictionary | High | TBD | TBD | Open |
